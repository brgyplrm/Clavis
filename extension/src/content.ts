...
function findInputsInShadow(root: Node = document): HTMLInputElement[] {
...
function findFormsInShadow(root: Node = document): HTMLFormElement[] {
...
function getClosestComposed(element: Element, selector: string): Element | null {
...
let boundInputFocus: any = null;
let boundInputClick: any = null;
let boundDocumentClick: any = null;
let boundWindowScrollResize: any = null;

function getLoginFormFields(): HTMLInputElement[] {
  const passwordFields = findInputsInShadow().filter(input => input.type === 'password');
...
function setupDropdownListeners(matches: any[]) {
...
function showDropdown() {
  ...
  currentMatches.forEach(entry => {
    ...
    item.addEventListener("click", (e) => {
      ...
      chrome.runtime.sendMessage({ type: "fill_entry", id: entry.id }, (response: any) => {
...
// Forms capture state
const formsAttached = new Set<HTMLFormElement>();
...
function monitorInputs() {
  ...
  passwordFields.forEach(passField => {
    if (passField.getAttribute("data-clavis-monitored") === "true") return;
...
function handleFormSubmission(container: HTMLElement) {
  ...
  if (!password) return; // Only capture if a password is typed
  ...
  chrome.runtime.sendMessage(
    ...
    (response: any) => {
      ...
      const alreadySaved = response.matches.some((match: any) => {
...
function checkPendingSave() {
  chrome.runtime.sendMessage({
    ...
    const err = chrome.runtime.lastError;
    if (err || !response || !response.pending) return;
...
function showSaveBanner(pending: any) {
  if (document.getElementById("clavis-save-banner-root")) return;
  ...
  const inputPassword = shadow.getElementById("input-password") as HTMLInputElement;
  const inputTitle = shadow.getElementById("input-title") as HTMLInputElement;
  const inputUsername = shadow.getElementById("input-username") as HTMLInputElement;
...
function isBlockedDomain(callback: (blocked: boolean) => void) {
  chrome.runtime.sendMessage({ type: "get_blocklist" }, (response: any) => {
    const err = chrome.runtime.lastError;
    ...
    const blocked = response.domains.some((blockedDomain: string) => {
...
function detectRegistrationForm(container: HTMLElement = document.body): boolean {
  ...
  for (const input of inputs) {
    ...
    if (
      id.includes("confirm") || name.includes("confirm") || placeholder.includes("confirm") || aria.includes("confirm") ||
...
function isLoginSequence(container: HTMLElement): boolean {
